...
def sum_vectors (vec1, vec2):
  """
  Silly little helper function to add two vectors and return their sum.
  """
  return_value = []

  if (len(vec1) != len(vec2)):
    sys.stderr.write("Unequal vectors (%d != %d).\n" % (len(vec1), len(vec2)))
    sys.exit(1)

  for index in range(0, len(vec1)):
    return_value.append(vec1[index] + vec2[index])

  return(return_value)
  
###############################################################################
def parse_mods(pepseq_with_mods):
    """
...
###############################################################################
def calc_proportion_fragments_incommon(peptide1,
                                           peptide2,
                                       binsize):
    """
    ...
    (pepseq1, peptide1_mods, nterm1, cterm1) \
       = parse_mods(peptide1)
...
def read_peptides(in_file_name):
  ...
  for line in inFile:
    ...
    if (len(words) > 1):
      # for old tide-index (separate target and decoy peptide files)
...
###############################################################################
"""
def main(in_file_name1, in_file_name2):
  global USAGE
  ...
  static_mods = {} # Key = amino acid, value = mass offset
  skip_same = False
  ...
  for index1 in range(0, len(list1)):
    ...
    for index2 in range(start_index, len(list2)):
      ...
      if ((mz_thresh != 0.0) and (abs(mass_diff) <= mz_thresh)):
        ...
        num_printed += 1
        sys.stdout.write("%s\t%s\t%g"
                         % (peptide1, peptide2, similarity))
        if (mz_thresh != 0.0):
          sys.stdout.write("\t%.4f\t%.4f\t%.4f\t%.4f"
                           % (masses1[index1], masses2[index2],
                              masses1[index1] - masses2[index2],
                              mass_diff))
        sys.stdout.write("\n")

  sys.stderr.write("Printed %d out of %d pairs.\n"
                   % (num_printed, num_pairs))
